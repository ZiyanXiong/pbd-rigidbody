...
function [x, f, exitflag, output, lambda]= gpqp_staggered(G,c,l,u,x,opts,mu)
if nargin < 2
...
for iter = 1:iterNum
    ...
    [xZ,~,~,CGiterN,~] = pcg_rs(G(neqIndex, neqIndex), -cZ,eps,CGiterNum,W_zz,[],xZ,lNeq,uNeq,projectionOpts);
    ...
    if(~all(~dyanmicFricitonIndex))
        [xdf,neqIndexDynamicFriciotn] = computeCauchyPoint(G(dyanmicFricitonIndex,dyanmicFricitonIndex), ...
            ...
            l(dyanmicFricitonIndex), ...
            u(dyanmicFricitonIndex), ...
            x(dyanmicFricitonIndex));
        x(dyanmicFricitonIndex) = xdf;
        neqIndex(dyanmicFricitonIndex) = neqIndexDynamicFriciotn;
        neqIndex = neqIndex & dyanmicFricitonIndex;
        lNeq = l(neqIndex);
        uNeq = u(neqIndex);
        %xY = A * xc;
        xY = x(~neqIndex);
        ...
        %xZ = pcg_rs(Z' * G * Z, -cZ,eps,CGiterNum,W_zz,[],xZ,lNeq,uNeq,projectionOpts);
        [xZ,~,~,CGiterT,~] = pcg_rs(G(neqIndex, neqIndex), -cZ,eps,CGiterNum,W_zz,[],xZ,lNeq,uNeq,projectionOpts);
        CGiterVec = [CGiterVec, CGiterN+CGiterT];
        x(neqIndex) = xZ;
