classdef ShapeNonConvexObj < apbd.Shape
	%%
	properties
        fileNames  % .obj filenames (path)
        F  % face elements
        V  % vertices
        E_oi
        E_io
	...
	methods
		%%
        function this = ShapeNonConvexObj(filename_list)
			this = this@apbd.Shape();
            this.fileNames = filename_list;
        end

		%%
		function I = computeInertia(this,density)
			[this.V,this.F] = apbd.ShapeMeshObj.readOBJ(this.fileNames{1});
			...
			% Call VolInt
			[T0,T1,T2,TP] = apbd.ShapeMeshObj.VolumeIntegration(this.V,this.F);
			...
			V_ = this.E_io * [this.V;ones(1,nverts)];
            this.radius = max(vecnorm(V_(1:3,:)));
		...
		function flag = broadphaseGround(this,E,Eg)
			...
			xw = E*xl;
			xg = Eg\xw;
			r = this.radius; % dist to a corner
			flag = xg(3) < 1.2*r;
		end
		...
		function cdata = narrowphaseGround(this,E,Eg)
            cdata = [];
			nverts = size(this.V,2);
			xl = this.E_io * [this.V;ones(1,nverts)];
			xw = E*xl;
			xg = Eg\xw;
            depth = xg(3,:);
            maxdepth = min(depth);
            if(maxdepth < 0.2)
                cindices = find(depth < maxdepth + 5e-2);
                if(length(cindices) > 8)
                    subindices = zeros(1,4);
                    [~,subindices(1)] = min(xg(1,cindices));
                    [~,subindices(2)] = max(xg(1,cindices));
                    [~,subindices(3)] = min(xg(2,cindices));
                    [~,subindices(4)] = max(xg(2,cindices));
                    cindices = cindices(subindices);
                end
			    for i = cindices
		...
		function flag = broadphaseShape(this,E1,that,E2)
			if isa(that,'apbd.ShapeNonConvexObj')
				p1 = E1(1:3,4);
				p2 = E2(1:3,4);
				d = norm(p1 - p2);
				r1 = this.radius; % dist to a corner
				r2 = that.radius; % dist to a corner
				flag = d <= 1.2*(r1 + r2);
			else
		...
		function cdata = narrowphaseShape(this,E1,that,E2)
			...
			if isa(that,'apbd.ShapeNonConvexObj')
                ...
                np1 = length(this.fileNames);
                %np2 = length(that.fileNames);
                for i = 2:2:np1
                    for j = 1
				        collisions = coalMeshMesh(E1*this.E_io, this.fileNames{i}, E2*that.E_io, that.fileNames{j});
				        nw = collisions.nor; % The normal is outward from body 1 (red)
				        for k = 1 : min(collisions.count,2)
					        xw = collisions.pos(:,k);
					        d = collisions.depth(k); % odeBoxBox returns positive depth for hits
		...
		function [F,V] = draw(this,E,color,axisSize)
			...
			nverts = size(this.V,2);
			V = E * this.E_io * [this.V;ones(1,nverts)];
            V = V(1:3,:)';
            F = this.F';
