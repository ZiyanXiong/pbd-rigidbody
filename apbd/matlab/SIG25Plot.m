function SIG25Plot(modelID, animation)
    ...
    if(animation)
        for i = 1:steps
            f = figure('visible','off');
            semilogy(1:size(TGS_50_rVec,1), TGS_50_rVec, '-', 'DisplayName','Gauss-Seidel with 50 sub steps','linewidth',2);
            hold on;
            semilogy(1:size(TGS_150_rVec,1), TGS_150_rVec, '-' ,'DisplayName','Gauss-Seidel with 150 sub steps','linewidth',2);
            hold on;
            semilogy(1:size(TGS_500_rVec,1), TGS_500_rVec, '-' ,'DisplayName','Gauss-Seidel with 500 sub steps','linewidth',2);
            hold on;
            semilogy(1:size(TPSP_rVec,1), TPSP_rVec, '-' ,'DisplayName','2 Pass Shock Propagation','linewidth',2);
            ...
            f = figure('visible','off');
            plot(1:size(TGS_50_iterVec,1), TGS_50_iterVec, '-', 'DisplayName','Gauss-Seidel with 50 sub steps','linewidth',2);
            hold on;
            plot(1:size(TGS_150_iterVec,1), TGS_150_iterVec, '-', 'DisplayName','Gauss-Seidel with 150 sub steps','linewidth',2);
            hold on;
            plot(1:size(TGS_500_iterVec,1), TGS_500_iterVec, '-', 'DisplayName','Gauss-Seidel with 500 sub steps','linewidth',2);
            hold on;
            plot(1:size(TPSP_iterVec,1), TPSP_iterVec, '-', 'DisplayName','2 Pass Shock Propagation','linewidth',2);
            xline(i, '--k','HandleVisibility', 'off', 'LineWidth', 1.5);
            legend off;
            set(gca,'FontSize',22);
            title('');
        ...
        legend('GS-50','GS-150','GS-500','2PSP', 'Location', 'bestoutside');
        grid on;
        exportgraphics(f, strcat(plotFolder, sprintf("residual_per_timestep.tif")), 'Resolution',300);
        ...
        ax.YTick = logspace(-6,4,6); % Major ticks
        grid on;
        exportgraphics(f, strcat(plotFolder, sprintf("iteration_per_timestep.tif")), 'Resolution',300);
