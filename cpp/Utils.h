...
namespace _2psp {
    ...
    namespace math {
        ...
        inline Matrix3 get_tangent(const Vector3 normal){
			Matrix3 tangent;
			if (abs(normal(2)) < eps_big) {
				tangent.col(0) = Vector3::UnitZ();
			}
			else {
				tangent.col(0) = Vector3::UnitX();
			}
			tangent.col(2) = normal.cross(tangent.col(0));
            tangent.col(2).normalize();
			tangent.col(1) = -normal.cross(tangent.col(2));
            tangent.col(0) = normal;
			return tangent;
        }
    }
