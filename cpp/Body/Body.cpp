...
namespace _2psp {
    ...
    */
    void Body::update() {
        _E_0i.topLeftCorner(3, 3) = Quat(_q.segment<4>(0)).toRotationMatrix();
    ...
    void Body::step_unconstrained(se3& fm) {
        Matrix3 I_inv = _E_0i.topLeftCorner(3, 3) * _Inertia_inv.head<3>().asDiagonal() * _E_0i.topLeftCorner(3, 3).transpose();
        Matrix3 I;
        if (_Inertia_inv.head<3>().norm() > math::eps_big) {
			I = _E_0i.topLeftCorner(3, 3) * _Inertia_inv.head<3>().cwiseInverse().asDiagonal() * _E_0i.topLeftCorner(3, 3).transpose();
		}
        else {
			I.setIdentity(3, 3);
		}
        Vector3 t = fm.tail<3>() - _phi.segment<3>(3).cross(I * _phi.segment<3>(3));
        Vector3 f = fm.head<3>() + _sim->_options->_gravity / _mass_inv;
    ...
    Vector3 Body::transform_point(const Vector3& p) {
        ...
        Vector4 p4 = Vector4(p(0), p(1), p(2), 1.0);
        Vector4 p4_w = _E_0i * p4;
        return p4_w.head<3>();
    }
    ...
    void Body::interagate_state() {
        ...
        Vector3 dtheta_axis = _phi_dt.segment<3>(0).normalized();
        AngleAxis dAa(_phi_dt.segment<3>(0).norm(), dtheta_axis);
        _q.segment<4>(0) = (Quat(_q.segment<4>(0)) * dAa).normalized().coeffs();
        _q.segment<3>(4) += _phi_dt.segment<3>(3);
